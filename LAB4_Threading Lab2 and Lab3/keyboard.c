...
extern void	start_Thread_temp();
extern void	start_Thread_accel();
extern void segClearAll();
...
void switchThread()
{
	...
	if(currentThread == tid_Thread_accel)
	{
		setLedIntensityPitch(0);
		setLedIntensityRoll(0);
		start_Thread_temp();
		osThreadTerminate(tid_Thread_accel);
	}
	
	else if(currentThread == tid_Thread_temp)
	{
		segClearAll();
		start_Thread_accel();
		osThreadTerminate(tid_Thread_temp);
	}
